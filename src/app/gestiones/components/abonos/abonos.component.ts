...
import { AbonosDialog  } from './abonos-dialog/abonos-dialog.component';
...
export class AbonosComponent implements OnInit {
  ...
  displayedColumns: string[] = ['ID', 'TIPO_ABONO','icons'];
  ...
  applyFilterOne(filterValue) {
    ...
    deletePermiso(id) {
      console.log(id);
      this.cobranzaService.activate();
      this.asignacionService.abonoDr(id.ID).then(data => {
          this.cobranzaService.deactivate();
