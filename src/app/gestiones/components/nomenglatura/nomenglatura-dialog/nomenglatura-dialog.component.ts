...
@Component({
    selector: 'nomenglatura-dialog',
    templateUrl: 'nomenglatura-dialog.component.html',
})
export class NomenglaturaDialog implements OnInit {
    callCenterForm: FormGroup;
    listadoGestion = [];
    constructor(
        public dialogRef: MatDialogRef<NomenglaturaDialog>,
    ...
    onSubmit() {
        if(this.callCenterForm.valid) {
            this.asignacionService.NomenglaturaAdd(this.callCenterForm.getRawValue())
                .then((data:any) => {
                    if(data.Status) {
