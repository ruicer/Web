...
export class AsignacionComponent implements OnInit {
  ...
  displayedColumns: string[] = ['SUCURSAL', 'COD_SUCURSAL', 'COD_REGION', 'icons'];
  ...
  ngOnInit() {
    ...
    this.filteredOptions = this.callCenterForm.get('PROCURADORES').valueChanges
      .pipe(
        startWith(''),
  ...
  susbcribeValues() {
    ...
    this.callCenterForm.get('PROCURADORES').valueChanges.subscribe(data => {
      this.cobranzaService.activate();
  ...
  valuesChanges(procurador) {
    ...
    this.asignacionService.PermisosAsignados(procurador).then(values => {
      console.log(values);
    ...
    this.asignacionService.PermisosNoAsignados(procurador).then(values => {
  ...
  addPermiso(value) {
    ...
    const dataOb = {
      P_REGI: value.COD_REGION,
      P_SUC: value.COD_SUCURSAL,
      P_PRO: this.callCenterForm.get('PROCURADORES').value
    }
    this.cobranzaService.activate();
    this.asignacionService.PermisosAddAsig(dataOb).then((data: any) => {
      
      if(data.Status) {
        this.valuesChanges(this.callCenterForm.get('PROCURADORES').value);
  ...
  deletePermiso(value) {
    ...
    this.cobranzaService.activate();
    this.asignacionService.DrPermiso(this.callCenterForm.get('PROCURADORES').value, value.COD_SUCURSAL)
      .then((data: any) => {
        if(data.Status) {
          this.valuesChanges(this.callCenterForm.get('PROCURADORES').value);
