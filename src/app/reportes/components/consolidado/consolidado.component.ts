...
export class ConsolidadoComponent implements OnInit {
    ...
    displayedColumns: string[] = ['COD_CLIENTE', 'NOMBRE', 'CREDITO', 'IDENTIFICACION', 'DEPARTAMENTO',
        'MUNICIPIO', 'TOTALSALDO', 'NUM_EXPEDIENTE', 'SALDOCAPITAL', 'STATUSMORA'];
    ...
    generateReport() {
        this.router.navigate(['/reportes/consolidado'], { queryParams: { length: this.length, pageIndex: 1} });
    }
    ...
    getSubscribeParams() {
        ...
        this.route.queryParams.subscribe(queryParams => {
            ...
            this.reportService.getConsolidadoSkips(queryParams['pageIndex'])
                .then((data: any) => {
                    let creditos = [];
    ...
    getServerData(event)  : any{
        ...
        this.router.navigate(['/reportes/consolidado'],
    ...
    export() {
        if (this.callCenterForm.get('sucursal').value) {
            this._snackBar.openFromComponent(SnackComponent, {
                duration: 35000,
                data: 'cargando.. reporte consolidado.'
            });
            this.cobranzaService.activate();
            this.reportService.getConsolidado().subscribe(fileData => {
                ...
                if (link.download !== undefined) {
                    ...
                    link.setAttribute("download", 'reporteConsolidado.xls');
